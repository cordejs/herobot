...
export HEROBOTDB=herodb
export HEROUSER=hero

echo
echo "--------------------------------------------"
echo "This script will install PostgreSQL."
echo "and create Herobot database and user."
...
read -e -p "Create Herobot Database and user? [y/n] " -i "n" createdb
if [ "$createdb" = "y" ]; then
  sudo -u postgres createuser -D -A -P $HEROUSER
  sudo -u postgres createdb -O $HEROUSER $HEROBOTDB
fi
